...
def tsa(patches, context_images, context_labels, model, max_iter=40, lr=0.1, lr_beta=1, distance='cos'):
    """
    ...
    for i in range(max_iter):
        ...
        # patches = [[], [], [], []]
        #
        # for i, x in enumerate(concat_images):
        #     x = x.to(torch.uint8)
        #     img = to_pil(x)
        #     h, w = img.size
        #     ch = 0.3 * h
        ...
        # clustering branch
        # for way-clustering
        way = images_gather.shape[0] // 15
        label_clu = torch.LongTensor(
            (list(np.reshape([[j] * 5 for j in range(way)], (1, -1)).squeeze()) +
        ...
        loss2 = 0.5*loss_clu + 0.5*loss_loc
